...
class AMPS_FAST(nn.Module):
    ...
    -------------------------------
    Weight Cost:
        (n-1)*n//2 * bond_dim * bond_dim * phys_dim
       +         n * bond_dim * phys_dim
    '''
    def __init__(self, n=20, bond_dim=5, phys_dim = 2, std=1e-8):
    ...
    def forward(self, data):
        ...
        #can also only regist first tensor via (1,D) but than cannot use paraller accelaration
        #here give the chance to use much more information, for example, use diag(D,D) rather than first row
        #(1,n-1,B,D,D) -> (n-1,B,D,D)  -> (n-1,B,D)
        tensor = torch.einsum('nbd,ndp->nbp',tensor,self.diag[1:])
        #(n-1, B, D) <-> (n-1, D, 2)  -> (n-1,B,2)
        tensor = torch.cat([diag[0][0].expand_as(tensor[0]).unsqueeze(0),tensor]).transpose(1,0)
        # (n,D,2)->(,2)->(B,2)->(B,1,2) + (B,n-1,2)-> (n,B,2)-> (B,n,2)
        # the softmax act on the last 2 channal.
        # That is the AMPS give every pixel possibility one by one.
        # So it is suppose to be a pixel generator.
        return tensor
