...
def submatrix_test_full_contraction_einsum(D=5,L=5,W=None,B=1,H=None,
                                            ...
                                            path_recorder=None,
                                            contractor='torch'):
    if B>1: assert contractor == 'torch'
    if contractor == 'torch':
        contract_engine = torch.einsum
    elif contractor == 'oe':
        contract_engine = oe.contract
    else:
...
def submatrix_test_sequence_contraction_einsum(D=5,L=5,B=1,
    ...
    if contractor == 'torch':
        if B == 1:
            tensor_list   = [rd_engine(2,*l) for t in tn2D_shape_list for l in t]
        else:
        ...
        tensor_list= [rd_engine(*l) for t in tn2D_shape_list for l in t]
        operands   = structure_operands(tensor_list,sublist_list,outlist,type=contractor)
        edge1      = contract_engine(*operands,optimize=path).flatten(-2*L,-L-1).flatten(-L,-1)
        tensor_list= [rd_engine(*l) for t in tn2D_shape_list for l in t]
        operands   = structure_operands(tensor_list,sublist_list,outlist,type=contractor)
        edge2      = contract_engine(*operands,optimize=path).flatten(-2*L,-L-1).flatten(-L,-1)

    cent_tensor   = rd_engine(D,D,D,D) if B == 1 else  rd_engine(B,D,D,D,D)
    equation   = "ab,cdb,efcg,gha->hedf" if B == 1 else "kab,kcdb,kefcg,kgha->khedf"
...
def fullmatrix_test_with_tn(D=5,L=5):
    ...
    del node_list
    return out,f"{torch.cuda.memory_stats()['reserved_bytes.all.peak']/1024/1024}M"
def fullmatrix_test_directly(D=5,L=5,path_recorder=None,contractor='torch'):
    if contractor == 'torch':
        contract_engine = torch.einsum
    elif contractor == 'oe':
        contract_engine = oe.contract
    ...
    loss.backward()
    return out,f"{torch.cuda.memory_stats()['reserved_bytes.all.peak']/1024/1024}M"
def fullmatrix_test_corn_4(D=5,L=6,B=1,path_recorder=None,contractor='torch'):
    if contractor == 'torch':
        contract_engine = torch.einsum
    elif contractor == 'oe':
        contract_engine = oe.contract
...
def fullmatrix_test_RG(D=3,L=7,path_recorder=None,contractor='torch'):
    if contractor == 'torch':
        contract_engine = torch.einsum
    elif contractor == 'oe':
        contract_engine = oe.contract
    ...
    while True:
        ...
        for lu,ld,ru,rd in zip(lu_tensor,ld_tensor,ru_tensor,rd_tensor):
            equation = "abcd,kjal,efgb,hiej->khficgdl"
            ...
            out  = out.flatten(2,3).flatten(-4,-3).flatten(0,1).flatten(-2,-1)
    ...
    loss.backward()
    return out,f"{torch.cuda.memory_stats()['reserved_bytes.all.peak']/1024/1024}M"
def fullmatrix_test_boundary(D=3,L=7,W=None,H=None):
    ...
    for i in range(1,H//2):
        ...
        tensor=[torch.einsum("zabcd,zkjal->zkbjcdl",up,dw).flatten(-2,-1).flatten(2,3)
    ...
    if len(tensor_list)==1:
        #center (2,D,D,D,1) (2,D,D,D,D)  (2,D,D,D,D) (2,D,1,D,D)
        upper=[torch.einsum("abcd,kjal->kbjcdl",up,dw).flatten(-2,-1).flatten(1,2)
                                                for up,dw in zip(upper,tensor_list.pop())]
    tensor=[torch.einsum("abcd,aefg->fbecdg",up,dw).flatten(-2,-1).flatten(1,2)
                                                for up,dw in zip(upper,lower)]
    tensor= [t.squeeze(0).squeeze(-2) for t in tensor]
    tensor= tensor[::-1]
    out   = tensor[0]
    for t in range(1,len(tensor)):
        out = torch.einsum("ab,cb->ac",out,tensor.pop())
    out  = out.squeeze()
    loss = out.norm()
    loss.backward()
    return out,f"{torch.cuda.memory_stats()['reserved_bytes.all.peak']/1024/1024}M"
